...
struct TabButton: View {
  ...
  struct CShape1: Shape {
    func path(in rect: CGRect) -> Path {
      ...
      path.appendArc(
        withCenter: CGPoint(x: rect.maxX - 10, y: rect.maxY - 10), radius: 10, startAngle: 0,
        endAngle: 90, clockwise: true)
      ...
      path.appendArc(
        withCenter: CGPoint(x: rect.minX + 10, y: rect.minY + 10), radius: 10, startAngle: 180,
        endAngle: 270, clockwise: true)
  ...
  struct CShape: Shape {
    func path(in rect: CGRect) -> Path {
      ...
      path.appendArc(
        withCenter: CGPoint(x: rect.maxX - 25, y: rect.minY + 25), radius: 25, startAngle: 270,
        endAngle: 360, clockwise: false)

      // Right side from top to bottom with arc
      path.line(to: CGPoint(x: rect.maxX, y: rect.maxY - 25))
      path.appendArc(
        withCenter: CGPoint(x: rect.maxX - 25, y: rect.maxY - 25), radius: 25, startAngle: 0,
  ...
  struct CShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
      let path = UIBezierPath(
        roundedRect: rect, byRoundingCorners: [.topRight, .bottomRight],
  ...
  struct CShape1: Shape {
    ...
    func path(in rect: CGRect) -> Path {
      ...
      let path = UIBezierPath(
        roundedRect: rect, byRoundingCorners: [.topLeft, .bottomRight],
