...
func getFileURLFormPathStr(dir: String, filename: String) -> URL {
  FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent(
    dir
...
func get_model_setting_templates() -> [ChatSettingsTemplate] {
  ...
  do {
    ...
    for tenplate_file in tenplate_files {
      model_setting_templates.append(
        parse_model_setting_template(template_path: templates_path + "/" + tenplate_file))
    }
    let documentsPath = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first
    let user_templates_path = documentsPath!.appendingPathComponent("model_setting_templates")
    try fileManager.createDirectory(
      at: user_templates_path, withIntermediateDirectories: true, attributes: nil)
    let user_tenplate_files = try fileManager.contentsOfDirectory(
      atPath: user_templates_path.path(percentEncoded: true))
    for template_file in user_tenplate_files {
      model_setting_templates.append(
        parse_model_setting_template(
          template_path: user_templates_path.path(percentEncoded: true) + "/" + template_file))
    }
  } catch {
    print(error)
  }
...
public func create_demo_chat() {
  do {
    ...
    fileManager.secureCopyItem(
      at: URL(fileURLWithPath: demo_chat_res_path),
      to: URL(fileURLWithPath: destinationURL.path + "/demo_chat.json"))
  } catch {
    print(error)
  }
}
...
public func get_chats_list() -> [[String: String]]? {
  ...
  do {
    ...
    let files = try fileManager.contentsOfDirectory(
      at: destinationURL, includingPropertiesForKeys: nil, options: .skipsHiddenFiles
    ).sorted(by: {
      let date0 = try $0.promisedItemResourceValues(forKeys: [.contentModificationDateKey])
        .contentModificationDate!
      let date1 = try $1.promisedItemResourceValues(forKeys: [.contentModificationDateKey])
        .contentModificationDate!
    ...
    for chatfile_url in files {
      ...
      if chatfile.contains(".json") {
        ...
        let tmp_chat_info = [
          ...
          "chat_style": info?["chat_style"] as? String
            ?? "DocC" /*,
    ...
    for modelfile in files {
      for ext in exts {
        if modelfile.lastPathComponent.hasSuffix(ext) {
          ...
          let tmp_chat_info = [
            "icon": icon, "file_name": modelfile.lastPathComponent, "description": "",
          ]
          res.append(tmp_chat_info)
          // }
        }
...
var exclude_from_settings_template_keys = [
  "lora_adapters",
  "model",
  "clip_model",
  "title",
...
func CreateChat(
  _ in_options: [String: Any], edit_chat_dialog: Bool = false, chat_name: String = "",
...
//}
  ...
  func save_image_from_library_to_cache(_ image: NSImage?) -> String? {
    do {
      ...
      let resized_img = image?.resizeMaintainingAspectRatio(
        withSize: NSSize(width: 1024, height: 1024))
      if resized_img == nil {
...
func get_path_by_short_name(_ short_name: String, dest: String = "models", check_exist: Bool = true)
  -> String?
{
...
func get_downloadble_models(_ fname: String) -> [DownloadModelInfo]? {
  ...
  do {
    ...
    let data = try Data(
      contentsOf: URL(fileURLWithPath: downloadable_models_json_path), options: .mappedIfSafe)
    let jsonResult = try JSONSerialization.jsonObject(with: data, options: .mutableLeaves)
    ...
    for row in jsonResult_dict! {
      let tmp_info = DownloadModelInfo(
        name: row["name"] as? String, models: row["models"] as? [[String: String]])
      res.append(tmp_info)
    }
    //        return jsonResult_dict
    return res
...
func save_chat_history(_ messages_raw: [Message], _ fname: String) {
  do {
    ...
    for message in messages_raw {
      var tmp_msg = [
        "id": message.id.uuidString as AnyObject,
