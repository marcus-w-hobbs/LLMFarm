...
private func generateIndexFromChunks() async {
  ...
  let elapsedTime = await clock.measure {
    ...
    await index.addItems(ids: folderTextIds, texts: folderTextChunks, metadata: folderTextMetadata)
    { _ in
      progressCurrent += 1
    }
...
public func removeFileFromIndex(fileName: String?, ragURL: URL?, onlyCheck: Bool = false) async
  -> Bool
{
...
public func addFileToIndex(
  fileURL: URL?, ragURL: URL?,
  currentModel: EmbeddingModelType,
  comparisonAlgorithm: SimilarityMetricType,
  chunkMethod: TextSplitterType
) async {
  ...
  if similarityIndex == nil {
    ...
    await BuildNewIndex(
      searchUrl: ragURL!.appendingPathComponent("docs"),
      chunkSize: chunkSize,
