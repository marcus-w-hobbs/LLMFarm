...
struct ChatSettingsView: View {
  ...
  init(
    add_chat_dialog: Binding<Bool>, edit_chat_dialog: Binding<Bool>,
  ...
  init(
    ...
    if chat_config!["clip_model"] != nil {
      ...
      if get_path_by_short_name(chat_config!["clip_model"]! as! String) != nil {
    ...
    if chat_config!["lora_adapters"] != nil {
      ...
      if adapters != nil && adapters!.count > 0 {
        ...
        if get_path_by_short_name(adapters![0]["adapter"]! as! String, dest: "lora_adapters") != nil
        {
    ...
    if chat_config!["repeat_penalty"] != nil {
      self._model_repeat_penalty = State(
        initialValue: chat_config!["repeat_penalty"] as? Float ?? 0)
    }
    ...
    if chat_config!["parse_special_tokens"] != nil {
      self._parse_special_tokens = State(
        initialValue: chat_config!["parse_special_tokens"] as! Bool)
    }
    ...
    self._chunkOverlap = State(
      initialValue: chat_config?["chunk_overlap"] as? Int ?? self.chunkOverlap)
    self._ragTop = State(initialValue: chat_config?["rag_top"] as? Int ?? self.ragTop)

    if chat_config!["current_model"] != nil {
      self._currentModel = State(
        initialValue: getCurrentModelFromStr(chat_config?["current_model"] as? String ?? ""))
    }
    if chat_config!["comparison_algorithm"] != nil {
      self._comparisonAlgorithm = State(
        initialValue: getComparisonAlgorithmFromStr(
          chat_config?["comparison_algorithm"] as? String ?? ""))
    }
    if chat_config!["chunk_method"] != nil {
      self._chunkMethod = State(
        initialValue: getChunkMethodFromStr(chat_config?["chunk_method"] as? String ?? ""))
    }
    ...
    HStack(spacing: 0) {
      ...
      GeometryReader { g in
        ...
        VStack {
          ...
          case 5:
            DocsView(
              docsDir: "documents/" + (self.chat_name == "" ? "tmp_chat" : self.chat_name)
                + "/docs",
              ragDir: "documents/" + (self.chat_name == "" ? "tmp_chat" : self.chat_name),
              chunkSize: $chunkSize,
              chunkOverlap: $chunkOverlap,
