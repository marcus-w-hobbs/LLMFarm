    ...
    if self.model_files.count > 0 {
      ...
      self._status = State(
        initialValue: FileManager.default.fileExists(
          atPath: getFileURLFormPathStr(dir: "models", filename: self.file_name).path)
          ? "downloaded" : "download")
    }
  }
    ...
    HStack {
      ...
      VStack(alignment: .leading, spacing: 1) {
        ...
        HStack {
                ...
                Button(model_info["Q"]!) {
                  ...
                  status =
                    FileManager.default.fileExists(
                      atPath: getFileURLFormPathStr(dir: "models", filename: file_name).path)
                    ? "downloaded" : "download"
                }
              }
            }
          ...
          //                    if download_url != ""{
          DownloadButton(
            modelName: $modelName, modelUrl: $download_url, filename: $file_name, status: $status
          )
          .frame(alignment: .trailing)
          .frame(maxWidth: 50)
          //                    }
