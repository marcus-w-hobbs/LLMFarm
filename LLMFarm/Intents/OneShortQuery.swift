...
@MainActor
func OneShortQuery(
  _ queryIn: String, _ chat: String, _ token_limit: Int,
  img_path: String? = nil,
  ...
  var query = queryIn
  guard let res = aiChatModel.load_model_by_chat_name_prepare(chat, in_text: query, attachment: nil)
  else {
  ...
  do {
    if !use_history {
      ...
      let requestMessage = Message(
        sender: .user, state: .typed, text: query, tok_sec: 0,
        attachment: img_path, attachment_type: "image")
      aiChatModel.messages.append(requestMessage)

      aiChatModel.chat?.model?.contextParams.state_dump_path =
        get_state_path_by_chat_name(chat) ?? ""
    }
    ...
    try ExceptionCather.catchException {
      do {
        _ = try aiChatModel.chat?.model?.predict(
          query,
          {
            str, time in
